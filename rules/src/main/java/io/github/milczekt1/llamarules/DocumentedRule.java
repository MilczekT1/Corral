 ...
 *
 * <p>Pairing is the point: {@link #published()} reads the doc and the raw rule off the same object,
 ...
 * }</pre>
 *
 * <p><strong>Declare the {@code @ArchTest} field last.</strong> It runs {@code published()} during
...
public interface DocumentedRule {
    ...
    /** The raw, unfrozen rule. Test <em>this</em>: {@link #published()} seeds and passes. */
    ArchRule definition();

    RuleDoc doc();

    /**
     * What consumers evaluate. Registers the doc so the failure formatter can find the prose, pins
