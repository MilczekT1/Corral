 ...
 *
 * <p>A rule lands here rather than in a group when it is sound but not right for every codebase: a
 * group is adopted whole, so a rule inside one is on by default for everybody who wires that group.
 * Consumers opt in by naming the rule class — {@code ArchTests.in(TheRule.class)} — which is exactly
 * what a group's field does, so nothing about the rule differs.
 *
