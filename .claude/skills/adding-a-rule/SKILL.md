...
6. **Register it in a group** as an `@ArchTest ArchTests` field (see `TestingRulesGroup`). A rule
   ...
   A group is adopted whole, so everything in one runs for whoever wires it. A rule a team can
   reasonably decline stands alone instead, in **no** group: list it on `EveryStandaloneRule` (test
   sources) so the grammar, doc and id-uniqueness tests still walk it, give it a row in the
   **Standalone rules** table of `docs/rules.md` rather than the main one, and leave
