...
no group at all and are wired one at a time — they are in [Standalone rules](#standalone-rules)
below.
...
| `corral.jakarta.no-javax-validation` | `JakartaMigrationRulesGroup` | No class may depend on anything in `javax.validation..` — Bean Validation moved to `jakarta.validation..` in Jakarta EE 9, and a validator ignores constraints from the other namespace rather than rejecting them, so a `javax` `@NotNull` under Hibernate Validator 8 lets `null` through with no error. One bytecode dependency check over production and test classes alike: constraint annotations, `ConstraintValidator` implementations and calls on a `ConstraintViolationException` all count, one violation per dependency. Matched by package string, so a consumer without the legacy API still runs it. Not seen: a `catch (javax.validation.ConstraintViolationException e)` that calls nothing on `e` — a catch clause alone records no dependency — and constraints declared in a `validation.xml` mapping, which are not bytecode. |
...
one runs for everybody who wires it, and these are rules a team can reasonably decline. Wire one by
naming the rule class, which is exactly what a group's own field does — same freezing, same
exclusions, same failure output.
...
These rows are checked by the same test as the table above, on the id set and on the class named in
column 2.
