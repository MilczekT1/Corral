...
| `corral.test.no-thread-sleep` | `TestingRulesGroup` | A test class must not call `Thread.sleep` or `TimeUnit.sleep`. Matched by call target owner and name, so every overload counts, and scoped to test classes — a `Sleeper` helper or a `pause()` on a base test class is flagged where it is declared, provided it compiles into test output. |
